...
public class SysLoginController extends AbstractController {
    ...
    public void captcha(@PathVariable("time") String time, HttpServletResponse response) {
        ...
        if (redisTemplate != null) {
            ...
            caotchaCache.put(Constant.NUMBER_CODE_KEY + time, text + "|" + (System.currentTimeMillis() / 1000 + 60));
        }
    ...
    public Map<String, Object> login(String username, String password, String captcha, String randomStr) {
        String code_key;
        if (redisTemplate != null) {
            code_key = (String) redisTemplate.opsForValue().get(Constant.NUMBER_CODE_KEY + randomStr);
        } else {
            code_key = caotchaCache.get(Constant.NUMBER_CODE_KEY + randomStr);
            if (code_key != null) {
                String[] data = code_key.split("\\|");
                if (System.currentTimeMillis() / 1000 - Long.valueOf(data[1]) <= 60) {
                    code_key = data[0];
                } else {
                    code_key = null;
                }
